...
renderer.code = (
  ...
  return `<pre class="code-block"><code class="${langClass}">${
    escaped ? code : code
  }</code></pre>`;
};
  ...
  return (
    <div className="space-y-4">
      {concepts.map((concept, index) => {
        ...
        return (
          <div key={"id" in concept ? concept.id : index} className="card">
            <h4>{concept.concept_title}</h4>
            <div
              className="text-primary-500 text-sm mb-3 max-w-none"
              dangerouslySetInnerHTML={renderMarkdown(concept.concept_summary)}
            />
            <hr className="border-primary-50 border my-2" />
              ...
              <div className="mt-2">
                ...
                <ul className="space-y-1 ml-4">
                  {examples.map((example, exIndex) => (
                    <li
                      key={exIndex}
                      className="text-sm text-primary-400 list-disc font-light"
                    >
                      <div
